  ...
  class PersistentClient extends EventEmitter {
    ...
    updateDevice (device) {
      ...
      if (that._client  === undefined) {
        log.info("Received keep alive for disconnected device %s", that.device.name);
        needReconnect = true;
      }
      if (that._host != device.host) {
        log.info("New URL for device %s: %s", that.device.name, that.device.host);
        that._host  = device.host;
        needReconnect = true;
      }
      if (that._name != device.name) {
        log.info("New name for device: %s", that.device.name);
        that._name  = device.name;
        needReconnect = true;
      }
      if (that._port != device.port) {
        log.info("New port for device %s: %s", that.device.name, that.device.port);
        that._port  = device.port;
        needReconnect = true;
      }
      if (needReconnect) {
        log.info("Reconnecting %s to %s:%s", that.device.name, that.device.host, that.device.port);
        that._connectClient();
      } else {
        log.debug("No need to reconnected for %s", that.device.name);
      }
    }
