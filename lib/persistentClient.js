  ...
  class PersistentClient extends EventEmitter {
    ...
    _close () {
      ...
      if (that._connecting_client) {
        log.error("%s - CLEAN UP CONNECTING", that._name);
      ...
      if (that._client) {
        ...
        try
        {
          log.error("%s - CLEAN UP", that._name);
    ...
    _connectClient () {
      ...
      if (that._connecting_client || that._client) {
        log.debug("%s - Connection already open", that._name);
        return;
      }
      ...
      let launchRetry = function(){
        ...
        if (connectionRetries <= MAX_CONECTIONS_RETRIES) {
          ...
          log.error("%s -  CONNECT - %s", that._name, that.toString());
